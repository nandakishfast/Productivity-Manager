...
def execute(string_query, tuple=None):
    global received_data_from_server
    if(tuple==None):
        s.send(bytes(string_query,"utf-8"))
        print("sending below query for processing")
        print(string_query)
    else:
        q_splitted = string_query.split('?')
        query = ''
        for i in range(0,len(tuple)):
            query += (q_splitted[i] + "'" + str(tuple[i]) + "'")
        if(len(q_splitted)>len(tuple)):
            query += q_splitted[-1]
    ...
    if(response != 'success'):
        print('did not get a success response from server')
        tkinter.messagebox.showerror('Error', 'Something went wrong at server side, please close everything')
    else:
        print('got success msg from server, and got below data')
        msg = 'ok'
        s.send(bytes(msg,"utf-8"))


        buff_size = s.recv(1024).decode("utf-8")

        msg = 'ok'
        s.send(bytes(msg,"utf-8"))

        recvd_data = s.recv(int(buff_size))
        received_data_from_server = pickle.loads(recvd_data)

        print(received_data_from_server)

def fetchall():
    temp = copy.deepcopy(received_data_from_server)
    return temp
...
def CompletedActivity(user_id):
    ...
    def addActivityPointsForTheDay(user_id, date_, activity_points):
        ...
        execute('SELECT user_id FROM USER WHERE user_id = ?', (user_id,))
        result = fetchall()
        ...
        if len(result) == 0:
            ...
            execute('SELECT date_ FROM POINTS WHERE date_ = ? AND user_id = ?', (date_, user_id))
            result = fetchall()

            if len(result) == 0:
                execute('INSERT INTO POINTS VALUES (?,?,?,?)', (user_id, date_, 0, activity_points))
            else:
                execute(
    ...
    def addFollowUpPointsForTheDay(user_id, date_, follow_up_points):
        ...
        if len(result) == 0:
            ...
            if len(result) == 0:
                execute('INSERT INTO POINTS VALUES (?,?,?,?)', (user_id, date_, follow_up_points, 0))
            else:
                execute(
    ...
    def activity_category(passed_parameter):
        ...
        def clicked_ok_from_category():
            ...
            def enteractivity(userAndActiv):
                try:
                    ...
                    def doActivity(abc):
                        ...
                        if len(result) == 0:
                            ...
                            execute('SELECT COUNT(activity_register) FROM ACTIVITIES_DONE')
                            result = fetchall()
                            ...
                            execute('SELECT points FROM LIST_OF_ACTIVITIES WHERE activity_id = ?',
                                        (activity_id,))
                            result = fetchall()
                            ...
                            if external_link != '':  # add follow upp
                                ...
                                if activity_id == 25:
                                    ...
                                    arr = temparr[1].split("&")
                                    execute('SELECT book_id FROM BOOK WHERE book_id = ?', (book_id,))
                                    result = fetchall()
                                    if len(result) == 0:
                                        hasError = True
                                ...
                                if activity_id == 26:
                                    execute('INSERT INTO ARTICLES(activity_register,article_link) VALUES (?,?)',
                ...
                execute("SELECT activity_name FROM LIST_OF_ACTIVITIES WHERE activity_id=?", (activity_id,))
                result = fetchall()
                act_name = result[0][0]
            ...
            resultTemp = []
            if v1.get() == 1:
                execute("SELECT * FROM LIST_OF_ACTIVITIES WHERE category_id=1")
                resultTemp = fetchall()
                result += resultTemp

            if v2.get() == 1:
                execute("SELECT * FROM LIST_OF_ACTIVITIES WHERE category_id=2")
                resultTemp = fetchall()
                result += resultTemp

            if v3.get() == 1:
                execute("SELECT * FROM LIST_OF_ACTIVITIES WHERE category_id=3")
                resultTemp = fetchall()
                result += resultTemp

            if v4.get() == 1:
                execute("SELECT * FROM LIST_OF_ACTIVITIES WHERE category_id=4")
                resultTemp = fetchall()
    ...
    def codechef_status():
        ...
        def p_status():
            execute('SELECT status_id FROM CODECHEF WHERE problem_tag= ? AND activity_register IN (SELECT activity_register FROM ACTIVITIES_DONE WHERE user_id = ?)ORDER BY status_id ',
    ...
    def cp_status():
        ...
        def p_status():
            execute('SELECT status_id FROM CP_WEBSITES WHERE problem_link= ? AND activity_register IN (SELECT activity_register FROM ACTIVITIES_DONE WHERE user_id = ?) ORDER BY status_id',
    ...
    def article_follow_up():
        ...
        def fudone(p):
            ...
            if len(result__) == 0:
                ...
                execute('SELECT days_to_be_added FROM FOLLOW_UP_NUMBER WHERE follow_up_number_id = ?',
                            (follow_up_number,))
                result___ = fetchall()
                ...
                execute('SELECT points FROM LIST_OF_ACTIVITIES WHERE activity_id = 26')
                pts = fetchall()
                execute('SELECT repetition FROM ACTIVITIES_DONE WHERE activity_register = ?;',
                            (activity_register,))
                rep = fetchall()
    ...
    def word_follow_up():
        ...
        def fudone(p):
            ...
            if len(result10) == 0:
                ...
                execute('SELECT points FROM LIST_OF_ACTIVITIES WHERE activity_id = 27')
                pts = fetchall()
                execute('SELECT repetition FROM ACTIVITIES_DONE WHERE activity_register = ?;',
                            (activity_register,))
                rep = fetchall()
    ...
    def book_follow_up():
        ...
        def fudone(p):
            ...
            if len(result__) == 0:
                ...
                execute('SELECT points FROM LIST_OF_ACTIVITIES WHERE activity_id = 25')
                pts = fetchall()
                execute('SELECT repetition FROM ACTIVITIES_DONE WHERE activity_register = ?;',
                            (activity_register,))
                rep = fetchall()
        ...
        for i in range(len(result)):
            ...
            e1.grid(row=i+1, column=0, sticky='e')
            execute('SELECT page_no FROM BOOK_READ_REGISTER WHERE activity_register = ?', (result[i][1],))
            pages = fetchall()
    ...
    def new_book():
        ...
        def insert_new_book():
            ...
            execute('SELECT book_name FROM BOOK')
            temp_book_list = fetchall()
            book_list = []
            ...
            if str(book_name.get()).lower() not in temp_book_list:
                execute('INSERT INTO BOOK VALUES (?,?)', (book_rows, book_name.get()))
    ...
    def weight():
        ...
        def insert_weight():
            date_ = (cal.selection_get()).strftime("%Y-%m-%d")
            try:
                execute('INSERT INTO WEIGHT VALUES (?,?,?)', (date_, user_id, float(weight_entered.get())))
    ...
    def show_weight():
        ...
        def show_more():
            execute('SELECT date_entered,weight FROM WEIGHT WHERE user_id=? ORDER BY date_entered DESC', (user_id,))
            result = fetchall()
            dates = []
            weights = []
            for date in result:
        ...
        def show_less():
            execute('SELECT date_entered,weight FROM WEIGHT WHERE user_id=? ORDER BY date_entered DESC LIMIT 1',
                        (user_id,))
            result = fetchall()
            print(result)
            ...
            if weight_entered.get() == '':
                ...
                try:
                    ...
                    if 0 <= hour < 24 and 0 <= min < 60:
                        ...
                        execute("SELECT wake_up_time FROM wake_up_time where user_id = ? and date_ = ?;",
                                    (user_id, date_))
                        result = fetchall()
                        if not result:
                            execute(
    ...
    def show_wake():
        ...
        def show_more():
            execute('SELECT date_,wake_up_time FROM wake_up_time WHERE user_id=? ORDER BY date_ DESC', (user_id,))
            result = fetchall()
            dates = []
            weights = []
            for date in result:
        ...
        def show_less():
            execute('SELECT date_,wake_up_time FROM wake_up_time WHERE user_id=? ORDER BY date_ DESC LIMIT 1',
                        (user_id,))
            result = fetchall()
            print(result)
            last_date = result[0][0]
    ...
    def custom_activity():
        ...
        def insert_custom_activity():
            try:
                ...
                return
            execute('SELECT category_id FROM CATEGORY_OF_ACTIVITY WHERE category_name = ?',(cat_name,))
            result = fetchall()
            if(len(result)==0):
            ...
            execute('SELECT activity_id FROM LIST_OF_ACTIVITIES WHERE activity_name = ?',(activity_name,))
            result = fetchall()
            if(len(result)!=0):
                tkinter.messagebox.showinfo('Error', 'That activity already exists')
                return
            execute('SELECT count(activity_id) FROM LIST_OF_ACTIVITIES')
            result = fetchall()
            activity_id = int(result[0][0]) + 1
            execute('INSERT INTO LIST_OF_ACTIVITIES VALUES (?,?,?,?)',(activity_id,category_id,activity_name,act_pts))
...
class Login(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        def verify():
            #try:
                execute('SELECT * FROM USER WHERE user_name = ?', (name.get(),))
                result = fetchall()
                print(result)
                if not result:
                    raise EXCEPTION
...
class AccountCreation(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        def validate():
            ...
            print(res)
            present_user = []
            execute('SELECT COUNT(user_id) FROM USER')
            row_num = fetchall()
...
if __name__ == "__main__":
    ...
    try:
        print('Welcome to Productivity Manager. Please enter server IP')
        server_ip = input()
        ...
        response = s.recv(1024).decode("utf-8")
        if(response=='busy'):
            print('The server is currently busy, please try after some time')
            s.close()
            exit()
